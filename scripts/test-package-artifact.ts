...
async function waitForProcessGroupExit(
	...
	return waitForProcessGroupExitUntil(rootPid, Date.now() + timeoutMs);
}

async function waitForProcessGroupExitUntil(
	rootPid: number,
	deadline: number,
): Promise<boolean> {
...
async function terminateCommandProcessTree(
	...
	const proofDeadline = Date.now() + treeKillTimeoutMs;
	...
	if (usedDirectChildFallback) {
		await wait(Math.min(killGraceMs, Math.max(0, proofDeadline - Date.now())));
	...
	if (
		await waitForProcessGroupExitUntil(
			child.pid,
			Math.min(proofDeadline, Date.now() + killGraceMs),
		)
	) {
		return {
...
async function finalizeProductionWasm(): Promise<void> {
	...
	const marker = {
		...
		devInsecure: false,
		targets: ["nodejs", "web"],
		// WtEvent wire v3: session_id on session-scoped events, SessionClosed
		// tag 9 carrying the close reason, SessionDraining tag 10.
		eventWireVersion: 3,
	};
	await writeFile(
...
async function assertProductionWasm(): Promise<void> {
	...
	if (marker.eventWireVersion !== 3) {
		throw new Error(
			`WASM eventWireVersion mismatch: expected 3, got ${String(marker.eventWireVersion)}`,
		);
	}
