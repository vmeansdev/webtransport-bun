...
import { WebTransport, createServer, WebTransportError, E_HANDSHAKE_TIMEOUT } from "../src/index.js";
...
describe("parity error and close mapping (P4)", () => {
    ...
    test("closed resolves with WebTransportCloseInfo (closeCode, reason)", async () => {
        ...
        expect("closeCode" in info || "reason" in info).toBe(true);
    });
    ...
    test("constructor rejects unsupported options (allowPooling, requireUnreliable)", () => {
        expect(() => new WebTransport(`https://127.0.0.1:${port}`, { allowPooling: true })).toThrow(/unsupported option 'allowPooling'/);
        expect(() => new WebTransport(`https://127.0.0.1:${port}`, { requireUnreliable: true })).toThrow(/unsupported option 'requireUnreliable'/);
    });
    ...
    test("serverCertificateHashes: valid format throws not-supported, invalid format throws validation error", () => {
        ...
        expect(() =>
            new WebTransport(`https://127.0.0.1:${port}`, {
                serverCertificateHashes: [{ algorithm: "sha-384" as "sha-256", value: new Uint8Array(32) }],
            })
        ).toThrow(/only supports algorithm "sha-256"/);
    });

    test("createBidirectionalStream rejects sendOrder and sendGroup", async () => {
        const wt = new WebTransport(`https://127.0.0.1:${port}`, { tls: { insecureSkipVerify: true } });
        await wt.ready;
        await expect(wt.createBidirectionalStream({ sendOrder: 1 })).rejects.toMatchObject({
            message: expect.stringMatching(/unsupported option 'sendOrder'/),
        });
        await expect(wt.createBidirectionalStream({ sendGroup: 1 })).rejects.toMatchObject({
    ...
    test("createUnidirectionalStream rejects sendOrder and sendGroup", async () => {
        const wt = new WebTransport(`https://127.0.0.1:${port}`, { tls: { insecureSkipVerify: true } });
        await wt.ready;
        await expect(wt.createUnidirectionalStream({ sendOrder: 1 })).rejects.toMatchObject({
            message: expect.stringMatching(/unsupported option 'sendOrder'/),
        });
        await expect(wt.createUnidirectionalStream({ sendGroup: 1 })).rejects.toMatchObject({
            message: expect.stringMatching(/unsupported option 'sendGroup'/),
        });
        wt.close();
    });
