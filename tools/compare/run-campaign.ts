...
export function buildMeasuredArmArtifact(input: {
	...
	/**
	 * F4 binding for the lock reservation. Same shape as the
	 * capability binding: the campaign passes the supervisor's
	 * per-host digests so the artifact's per-host lock entries
	 * are checked against them; a self-attested lock digest
	 * cannot pass the binding
	 * `assertMeasuredArmObservedItsLock` enforces. Optional on
	 * the seam so a test that exercises the verdict / ledger
	 * mechanics without going through the resident loop can
	 * fabricate the seam; required in the campaign flow.
	 */
	readonly lockDigest?: {
	...
	return buildRunArtifact({
		...
		// F4 binding for the lock reservation, the same shape as the
		// per-host capability binding above. The campaign passes the
		// supervisor's per-host digests, the artifact's per-host lock
		// entries must match, and a self-attested lock digest fails
 ...
 *
 * Re-exported from `secure-fs.ts`. The anchor set lives there because both
 * the gate (`tools/compare/output-policy.ts`, roleChildTs) and the
 * orchestrator (`run-campaign.ts`, officialRoots) need to read it, and
 ...
 *
 * Thin wrapper over `secure-fs.ts`'s `selectMintingAnchor` that throws a
 * `ComparisonCliError` on ambiguity so callers in `run-campaign.ts` get
 * the campaign-scoped error code rather than a bare `Error`.
 */
export function selectMintingAnchor(
	anchors: readonly CampaignAuthorityAnchor[],
): string {
	try {
		return selectMintingAnchorFromSecureFs(anchors);
	} catch {
...
export function isPinnedCampaignAuthority(digest: unknown): digest is string {
	return isPinnedCampaignAuthorityFromSecureFs(digest);
}
