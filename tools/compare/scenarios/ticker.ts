...
export interface TickerScenarioConfig {
	readonly runId: string;
	readonly ingressRatePerSecond: number;
	readonly recordBytes: number;
	readonly durationSeconds: number;
...
/**
 * Pure-driver simulated ticker fanout runner for testing overload and completeness.
 */
export async function runTickerFanoutPure(
	config: TickerScenarioConfig,
): Promise<TickerScenarioResult> {
	const clock = config.clock ?? {
		now: () => Date.now(),
		sleep: async (ms) => new Promise((r) => setTimeout(r, ms)),
	};

	const ledger = createTickerLedger(config);
	const totalRecords = config.ingressRatePerSecond * config.durationSeconds;
	const dropRatio = config.dropRatio ?? 0;

	for (let i = 1; i <= totalRecords; i++) {
		const sendTime = clock.now();
		ledger.recordOffered(i, sendTime);
		ledger.recordServerObserved(i, sendTime + 1);

		// Broadcast to fanout subscribers
		for (let s = 1; s <= config.fanout; s++) {
			// Deterministic drop check for simulation testing
			if (dropRatio > 0) {
				const dropStep = Math.round(1 / dropRatio);
				if (dropStep > 0 && (i * config.fanout + s) % dropStep === 0) {
					continue;
				}
			}
			ledger.recordDelivered(`sub-${s}`, i, sendTime + 3);
		}

		if (i % config.ingressRatePerSecond === 0) {
			await clock.sleep(1000 / config.ingressRatePerSecond);
		}
	}
