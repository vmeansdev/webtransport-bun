...
test.describe("P3.3 interop expansion", () => {
    test("reconnect storm: rapid connect/close cycles complete without hang", async ({ page }) => {
    ...
    test("close with code and reason propagates to client", async ({ page }) => {
        ...
        expect(result.code === 4001 || result.reason?.includes("interop") || result.reason?.includes("Connection")).toBe(
            true
        );
    });
