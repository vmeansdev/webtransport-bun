#!/bin/bash
# The Mac generator's ssh entrypoint — G6's variant.
#
# A separate file, not an edit of mac-generator-entry.sh, for one reason: the
# sibling's sha256 is pinned by G2's PD-1 pre-flight (e21c43eb…), and G6 must
# not move a hash another gate's dispatch gate reads. The G6 deltas are:
# `mmo-client` joins the closed --bin set, and nothing else — the candidate
# rule, the watchdog, and the provenance contract are the sibling's, verbatim.
#
...
export PATH="${WT_MACGEN_PATH:-$HOME/.cargo/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin}"
...
case "$BIN_NAME" in
  load-client|broadcast-client|mmo-client) ;;
  *) echo "macgen: --bin must be load-client, broadcast-client or mmo-client, got '$BIN_NAME'" >&2; exit 3 ;;
esac
...
echo "macgen: binary=$BIN sha256=$(shasum -a 256 "$BIN" | awk '{print $1}')"
