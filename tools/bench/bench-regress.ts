...
async function runStreamBench(): Promise<{ throughput_mbps: number } | null> {
  const proc = await $`cd ${ROOT} && bun tools/bench/stream-throughput.ts`.quiet().nothrow();
...
async function main() {
  ...
  if (streamResult.throughput_mbps < STREAM_MIN_MBPS) {
    console.error(`bench-regress: stream throughput ${streamResult.throughput_mbps} MB/s below threshold ${STREAM_MIN_MBPS}`);
    process.exit(1);
  }
