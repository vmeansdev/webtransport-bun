...
const RSS_TREND_OUT =
	process.env.RSS_TREND_OUT ?? join(ROOT, "tools/load/rss-trend.json");
...
async function main() {
	...
	const poller = (async () => {
		for (
			let i = 0;
			i < Math.ceil((DURATION + 10) / (pollIntervalMs / 1000));
			i++
		) {
			...
			if (m.queuedBytesGlobal > maxQueuedBytesGlobal) {
				console.error(
					"load-addon: FAIL (queuedBytesGlobal",
					m.queuedBytesGlobal,
					"> max)",
				);
				await server.close();
	...
	if (m.sessionTasksActive > 10 || m.streamTasksActive > 10) {
		console.error(
			"load-addon: FAIL (task gauges high:",
	...
	if (m.queuedBytesGlobal > 128 * 1024) {
		console.error(
			"load-addon: FAIL (queuedBytesGlobal not baseline:",
			m.queuedBytesGlobal,
			")",
		);
		process.exit(1);
	...
	if (rssSamples.length > 0) {
		...
		const csv = [
			"ts_ms,rss_mb,sessions,streams",
			...rssSamples.map(
				(s) => `${s.ts_ms},${s.rss_mb.toFixed(2)},${s.sessions},${s.streams}`,
			),
		].join("\n");
