...
async function main() {
	...
	console.log(
		"overload-addon: Starting addon server (createServer, maxSessions=" +
			MAX_SESSIONS +
			")...",
	);
	...
	console.log(
		"overload-addon: Running load-client (" +
			SESSIONS_ATTEMPT +
			" sessions, " +
			DURATION +
			"s)...",
	...
	const client = Bun.spawn(
		[
			...
			"1",
			"--max-session-errors",
			String(SESSIONS_ATTEMPT - MAX_SESSIONS),
		],
		{
			cwd: ROOT,
			stdout: "inherit",
	...
	if (m.sessionsActive > MAX_SESSIONS + 2) {
		console.error(
			"overload-addon: FAIL (sessionsActive",
			m.sessionsActive,
			"> max",
			MAX_SESSIONS,
			"+ 2)",
	...
	if (m.limitExceededCount === 0) {
		console.error(
			"overload-addon: FAIL (limitExceededCount should be > 0 when shedding)",
		);
		process.exit(1);
	}
