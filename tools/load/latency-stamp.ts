 ...
 * | 44     | 1    | datagram class — version 3 only          |
 * | 45     | 3    | reserved, zero                           |
 *
...
export type StampInput = {
	...
	holdNs?: number;
	klass?: number;
	/** 2 (default) or 3. A version-3 stamp needs `STAMP_BYTES_V3` of room. */
	version?: number;
};
...
export function encodeStamp(bytes: Uint8Array, stamp: StampInput): void {
	...
	view.setUint8(OFFSET_CLASS + 1, 0);
	view.setUint8(OFFSET_CLASS + 2, 0);
	view.setUint8(OFFSET_CLASS + 3, 0);
}
...
export function writeReflection(
	...
	view.setUint16(OFFSET_VERSION, 3, true);
