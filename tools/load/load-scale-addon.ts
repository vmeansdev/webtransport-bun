...
async function main() {
	...
	if (!existsSync(clientBin)) {
		...
		console.log(
			"load-scale-addon: Using existing load-client:",
			clientBin.replace(`${ROOT}/`, ""),
		);
	}

	console.log(
		"load-scale-addon: Starting addon server, sessions=",
		SESSIONS,
		"duration=",
		DURATION,
	);
	...
	const poller = (async () => {
		for (
			let i = 0;
			i < Math.ceil((DURATION + 30) / (pollIntervalMs / 1000));
			i++
		) {
			...
			if (m.queuedBytesGlobal > maxQueuedBytesGlobal) {
				console.error(
					"load-scale-addon: FAIL (queuedBytesGlobal",
					m.queuedBytesGlobal,
					"> max)",
				);
				await server.close();
	...
	if (result.code !== 0) {
		if (peakSessions === 0 && peakStreams === 0) {
			console.error(
				"load-scale-addon: FAIL (load-client exited",
				result.code,
		...
		console.warn(
			"load-scale-addon: WARN (load-client exited",
			result.code,
	...
	console.log(
		"load-scale-addon: PASS (sessions=",
		m.sessionsActive,
		"streams=",
		m.streamsActive,
		"limitExceeded=",
