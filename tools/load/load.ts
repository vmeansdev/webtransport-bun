...
async function getFdCount(pid: number): Promise<number> {
	try {
		...
		console.warn("load: failed to count file descriptors:", err);
		return 0;
	}
}
...
async function getMetrics(): Promise<{
	...
	try {
		...
		console.warn("load: failed to fetch metrics snapshot:", err);
		return null;
	}
}
...
async function main() {
	...
	for (let i = 0; i < 30; i++) {
		try {
			...
			console.warn("load: health check attempt failed:", err);
			await Bun.sleep(200);
		}
		if (i === 29) {
			server.kill();
	...
	const getServerRss = async (): Promise<number> => {
		try {
			...
			console.warn("load: failed to read server RSS:", err);
			return 0;
		}
	};
	...
	const poller = (async () => {
		...
		while (Date.now() - start < (DURATION + 5) * 1000) {
			...
			if (m && m.queuedBytesGlobal > maxQueuedBytesGlobal) {
				console.error(
					"load: FAIL (queuedBytesGlobal",
					m.queuedBytesGlobal,
					"> max",
					maxQueuedBytesGlobal,
					")",
				);
				server.kill();
				process.exit(1);
	...
	console.log(
		...
		"(metrics:",
		finalMetrics
			? `sessions=${finalMetrics.sessionsActive} streams=${finalMetrics.streamsActive}`
			: "n/a",
		")",
	);
}
