...
async function getMetrics(): Promise<{ sessionsActive: number; streamsActive: number; queuedBytesGlobal: number } | null> {
    try {
        const res = await fetch("http://127.0.0.1:4434/metrics");
        if (!res.ok) return null;
        const j = (await res.json()) as { sessionsActive?: number; streamsActive?: number; queuedBytesGlobal?: number };
        return {
...
async function main() {
    ...
    const poller = (async () => {
        ...
        while (Date.now() - start < (DURATION + 60) * 1000) {
            ...
            if (m && m.queuedBytesGlobal > maxQueuedBytesGlobal) {
                console.error("soak: FAIL (queuedBytesGlobal", m.queuedBytesGlobal, "> max)");
                server.kill();
                process.exit(1);
            }
    ...
    if (initialFd > 0 && finalFd > initialFd * 2) {
        console.error("soak: FAIL (FD count", initialFd, "->", finalFd, "exceeds 2x)");
        process.exit(1);
    }
