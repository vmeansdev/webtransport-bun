...
/**
 * The registered data-path /24, as a dotted prefix ("10.99.0" by default —
 * the home cable). A rig registered on another private path (a rented VPC)
 * declares its prefix through `LATENCY_RTT_DATA_SUBNET`, and its registration
 * pins the value: the guard's job is refusing the *wrong* wire, and which
 * wire is right is a registration fact, not a code constant. The family-LAN
 * refusal is unconditional — that address family is never a data path.
 */
export function dataSubnetPrefix(env: NodeJS.ProcessEnv = process.env): string {
	const declared = (env.LATENCY_RTT_DATA_SUBNET ?? "").trim();
	if (declared === "") return "10.99.0";
	if (!/^10\.\d{1,3}\.\d{1,3}$/.test(declared) || declared === "192.168.2") {
		throw new Error(
			`LATENCY_RTT_DATA_SUBNET=${declared} must be a dotted 10.x.y /24 prefix`,
		);
	}
	return declared;
}
...
export function assertCableHost(host: string, envName: string): string {
	...
	const prefix = dataSubnetPrefix();
	const hostRe = new RegExp(`^${prefix.replaceAll(".", "\\.")}\\.\\d{1,3}$`);
	if (!hostRe.test(host)) {
		throw new Error(
			`${envName}=${host} is not a ${prefix}.0/24 data-path address — the ` +
