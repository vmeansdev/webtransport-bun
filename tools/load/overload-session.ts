...
async function getMetrics(): Promise<{
    ...
    try {
        ...
        const j = (await res.json()) as { sessionsActive?: number; limitExceededCount?: number };
        return {
...
async function main() {
    ...
    try {
        ...
        for (const p of [p4434.trim(), p4433.trim()].flatMap((s) => s.split(/\s+/).filter(Boolean))) {
    ...
    console.log("overload-session: Starting reference server (max_sessions=5)...");
    ...
    if (!m) {
        console.error("overload-session: FAIL (could not fetch metrics from http://127.0.0.1:4434/metrics)");
        process.exit(1);
    }

    const pass = m.sessionsActive <= MAX_SESSIONS + 2 && m.limitExceededCount > 0;
    console.log("overload-session: sessionsActive=", m.sessionsActive, "limitExceededCount=", m.limitExceededCount);

    if (m.sessionsActive > MAX_SESSIONS + 2) {
        console.error("overload-session: FAIL (sessionsActive", m.sessionsActive, "> max", MAX_SESSIONS, ")");
        process.exit(1);
    }
    if (m.limitExceededCount === 0) {
        console.error("overload-session: FAIL (limitExceededCount should be > 0 when shedding)");
        process.exit(1);
    }
