...
| Security / auth (CA roots) | wasm client `caPem` trust over user-supplied roots | `crates/wasm/src/verify.rs`; `packages/webtransport/src/backend.ts` | `packages/webtransport/test/wasm-ca-trust.test.ts` | **Implemented, construction-verified only — NOT end-to-end proven.** The tests assert the CA-verified endpoint builds and that malformed roots map to `E_TLS`; none completes a handshake against a CA-verified server, and an attempt to do so fails. Blocked on test infrastructure: `wt_generate_cert` emits self-signed leaves only, and a self-signed leaf cannot be its own webpki trust anchor. Open follow-up: test-only CA + leaf chain generation (rcgen can issue a leaf from a CA) plus an end-to-end handshake test. Treat as implemented-unproven, not parity-closed |
...
| WASM candidate path (coupled 1.0 target) | `crates/wasm/src/{bridge.rs,cert.rs,endpoint.rs,event.rs,h3.rs,lib.rs,verify.rs,server_tls.rs,congestion.rs}`; `packages/webtransport/src/{backend.ts,backend-wasm.ts,wasm-webtransport.ts,webtransport-like-wasm.ts,send-scheduler.ts,wasm-endpoint-pool.ts,ticket-store-hosts.ts,wasm-server-session.ts}` | `packages/webtransport/test/{wasm-*.test.ts,parity-*.test.ts}`; `tools/interop/interop-evidence-wasm.json` | `wasm-dynamic-qpack` / `wasm-multi-session` / `wasm-0rtt` **passed** on candidate; `wasm-facade-parity` **demoted to pending** — its evidence was a session-map smoke, superseded by all 9 `parity-*.test.ts` running on both backends via `webtransport-bun/portable` (native 67/0; wasm 64 pass/3 skip/0 fail), which needs a candidate rebind before it can be stamped (pooling/waitUntilAvailable/getStats/CC/sendOrder/tickets/metrics/TLS/log/`WasmServerSession` landed) |
...
| IWA browser server proof | `examples/webtransport-wasm-iwa/{README.md,app.js,.well-known/manifest.webmanifest}`; `tools/interop/run-iwa.mjs` | `docs/release-evidence/cb0cb698…/iwa-direct-sockets.json` (+ `.release-evidence/iwa/*` bundles) | `iwa-direct-sockets` **passed** on candidate |
