...
- getStats: returns WebTransportConnectionStats backed by real quinn transport
  stats (wire bytesSent/bytesReceived, packetsSent/packetsReceived/packetsLost,
  smoothedRtt); datagram sub-dictionary fields not exposed by the runtime are 0
...
| Session lifecycle     | `closed` promise semantics                          | `implemented`  | `closed` resolves with `WebTransportCloseInfo` on clean close and **rejects on connect failure** (same error as `ready`), per W3C; `toCloseInfo` maps native `code`/`reason` to `closeCode`/`reason` | Client-initiated close propagates closeCode/reason; interop may show generic message on network loss | Covered by parity-error-close and parity-facade-lifecycle              |
...
| Stats                 | `getStats()` dictionaries                           | `implemented`  | `getStats()` returns real wire-level QUIC stats from quinn (`bytesSent`/`bytesReceived` = UDP payload bytes, `packetsSent`/`packetsReceived`/`packetsLost`, `smoothedRtt` in ms); falls back to facade byte tallies only if the native stats call is unavailable | Backed by `quinn::Connection::stats()`                                    | Surface datagram drop/expiry counters when the runtime exposes them       |
| Security/auth         | `serverCertificateHashes` behavior                  | `implemented`  | Per W3C intent the pin REPLACES CA chain/hostname verification (accepts self-signed pinned certs); guardrails enforce cert currently-valid and total validity window ≤ 14 days | Enforced in connect path with stable error mapping                       | Validate CI with pinned self-signed cert coverage                        |
