...
- **Scale evidence (two required hosted loopback lanes)**: `bun tools/load/distributed-scale.ts` runs the same multi-process, multi-port harness under two deliberately separate contracts. The strict 20-session `probe` lane exercises datagrams plus uni/bidi/reset/stop-sending stream semantics with zero operation errors and supplies handshake, datagram, and stream p99 evidence in `.release-evidence/load/scale-probe-artifact.json`. The 10k `drain-all` lane disables streams and supplies only session-scale admission/caps, datagram delivery, drain, final-zero-gauge recovery, and memory recovery evidence in `.release-evidence/load/scale-10k-artifact.json`; it does not prove stream behavior at 10k. Both keep the authoritative `1.25` charged-memory recovery limit and both artifacts are release-blocking. Hosted loopback has one server-observed source identity and neither artifact is multisource evidence. The harness can record external identities and prefixes when run in a real distributed environment, but child-process count is never treated as source diversity. After the steady live set reaches the configured session cap, a separate excess-admission phase must produce nonzero limit/rate shedding and recover to its pre-overload session/task gauge baseline. The artifacts record `peakLiveSessions`, `sourceIdentityCount`, `sourcePrefixCount`, `liveSetHeldMs`, `admissionShedCount`, `overloadEvidence`, applicable p99 metrics, `peakQueuedBytesGlobal`, `peakRssMb`, `closeDurationMs`, and `finalGauges`. Missing metrics required by a lane or missing RSS caps is a hard failure.
