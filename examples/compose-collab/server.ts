import { createServer, type ServerSession } from "../../packages/webtransport/src/index.js";
...
const certHashBase64 = createHash("sha256").update(new X509Certificate(certPem).raw).digest("base64");

const encoder = new TextEncoder();
const decoder = new TextDecoder();
const sessions = new Map<string, ServerSession>();
const RECENT_EVENTS_MAX = Number(process.env.RECENT_EVENTS_MAX ?? 200);
const recentEvents: Array<{ at: string; type: string; detail: Record<string, unknown> }> = [];
const counters = {
  datagramIn: 0,
  bidiIn: 0,
  uniIn: 0,
...
const wtServer = createServer({
  ...
  onSession: async (session) => {
    ...
    console.log(`[server] session joined id=${session.id} peer=${session.peer.ip}:${session.peer.port}`);
    ...
    void session.closed.then(async () => {
      ...
      pushEvent("session.leave", { sessionId: session.id, activeSessions: sessions.size });
      await sendDatagramToAll({
        type: "presence",
        action: "leave",
        sessionId: session.id,
...
const httpServer = Bun.serve({
  ...
  fetch(req) {
    ...
    if (url.pathname === "/healthz") {
      return Response.json({ ok: true, sessions: sessions.size, wtPort: WT_PORT, certHashBase64 });
    }
