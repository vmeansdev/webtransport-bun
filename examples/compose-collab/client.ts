...
const CLIENT_ID = process.env.CLIENT_ID ?? `client-${Math.floor(Math.random() * 1000)}`;
...
const presenceTicker = setInterval(() => {
  const payload = JSON.stringify({ type: "presence-ping", from: CLIENT_ID, at: nowIso() });
  void session.sendDatagram(encoder.encode(payload));
  log(`datagram sent: ${payload}`);
}, PRESENCE_MS);

const chatTicker = setInterval(async () => {
  const payload = JSON.stringify({ type: "chat", from: CLIENT_ID, text: `hello at ${nowIso()}` });
...
const uniTicker = setInterval(async () => {
  const payload = JSON.stringify({ type: "state", from: CLIENT_ID, cursor: Math.floor(Math.random() * 1000), at: nowIso() });
